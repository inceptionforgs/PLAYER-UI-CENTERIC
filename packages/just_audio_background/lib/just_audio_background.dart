...
class JustAudioBackground {
  /// App registers this so Android Auto can toggle Mewati Bass.
  static Future<bool> Function()? onToggleMewatiBass;
  static bool Function()? mewatiBassIsOn;

  /// Call after bass is toggled in-app so car display icon updates.
  static void notifyMewatiBassChanged() {
    try {
      _playerAudioHandler.rebroadcastControls();
    } catch (_) {}
  }
...
class _PlayerAudioHandler extends BaseAudioHandler
  ...
  Future<dynamic> customAction(String name,
    ...
    if (name == 'mewatiBass') {
      final on = await JustAudioBackground.onToggleMewatiBass?.call();
      _broadcastStateIfActive();
      return on ?? JustAudioBackground.mewatiBassIsOn?.call() ?? false;
    }
    return super.customAction(name, extras);
  }

  void rebroadcastControls() => _broadcastStateIfActive();
  ...
  void _broadcastState() {
    final bassOn = JustAudioBackground.mewatiBassIsOn?.call() ?? false;
    final bassControl = MediaControl.custom(
      androidIcon:
          bassOn ? 'drawable/ic_mewati_bass_on' : 'drawable/ic_mewati_bass_off',
      label: bassOn ? 'Mewati Bass On' : 'Mewati Bass',
      name: 'mewatiBass',
    );
    final controls = [
